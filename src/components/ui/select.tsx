  ...
  return (
    <SelectPrimitive.Portal>
      <SelectPrimitive.Content
        ...
        className={cn(
          '!bg-foreground-white text-foreground-black data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 data-[side=bottom]:slide-in-from-top-2 data-[side=left]:slide-in-from-right-2 data-[side=right]:slide-in-from-left-2 data-[side=top]:slide-in-from-bottom-2 relative z-50 max-h-(--radix-select-content-available-height) min-w-[8rem] origin-(--radix-select-content-transform-origin) overflow-x-hidden overflow-y-auto !rounded-xl border shadow-md',
          position === 'popper' &&
...
/** Scrollable dropdown for long / paginated lists — same collision / flip behavior as SelectContent. */
function SelectScrollContent({
  className,
  viewportClassName,
  children,
  position = 'popper',
  ...
  return (
    <SelectPrimitive.Portal>
      <SelectPrimitive.Content
        data-slot="select-scroll-content"
        className={cn(
      ...
      >
        ...
        <SelectPrimitive.Viewport
          className={cn(
            'max-h-[min(15rem,var(--radix-select-content-available-height))] overflow-y-auto overflow-x-hidden p-1',
...
function SelectLabel({ className, ...props }: React.ComponentProps<typeof SelectPrimitive.Label>) {
  return (
    <SelectPrimitive.Label
      ...
      className={cn('text-mute-foreground px-2 py-1.5 text-xs', className)}
      {...props}
    />
  );
}
...
function SelectSeparator({ className, ...props }: React.ComponentProps<typeof SelectPrimitive.Separator>) {
  return (
    <SelectPrimitive.Separator
      ...
      className={cn('bg-border-grey pointer-events-none -mx-1 my-1 h-px', className)}
      {...props}
    />
  );
}
